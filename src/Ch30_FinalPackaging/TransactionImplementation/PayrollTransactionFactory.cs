...
public class PayrollTransactionFactory : TransactionFactory.Factory {
    ...
    public TransactionApplication.Transaction MakeTimeCardTransaction(Ch30.CommonTypes.Date date, decimal hour,
        int empId) {
        return new TimeCardTransaction(date, hour, empId);
    }

    public TransactionApplication.Transaction MakeSalesReceiptTransaction(decimal amount, Ch30.CommonTypes.Date date,
        int empId) {
        return new SalesReceiptTransaction(amount, date, empId);
    }

    public TransactionApplication.Transaction MakeServiceChargeTransaction(int memberId, Ch30.CommonTypes.Date date,
        decimal amount) {
    ...
    public TransactionApplication.Transaction MakeChangeCommissionedTransaction(int empId, decimal salary,
    ...
    public TransactionApplication.Transaction MakePaydayTransaction(Ch30.CommonTypes.Date date) {
        return new PaydayTransaction(date);
    }

    public TransactionApplication.Transaction MakeAddCommissionedEmployeeTransaction(int empId, string name,
        string address, decimal salary, decimal commissionrate) {
        return new AddCommissionedEmployee(empId, name, address, salary, commissionrate);
    }

    public TransactionApplication.Transaction MakeAddSalariedEmployeeTransaction(int empId, string name, string address,
        decimal salary) {
        return new AddSalariedEmployee(empId, name, address, salary);
    }

    public TransactionApplication.Transaction MakeAddHourlyEmployeeTransaction(int empId, string name, string address,
        decimal hourlyRate) {
